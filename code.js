...
bot.on('message', async message => {
  ...
  if (message.content.toLowerCase().startsWith(`${config.prefix}code`) || message.content.toLowerCase().startsWith(`${config.prefix}link`) || message.content.toLowerCase().startsWith(`${config.prefix}tutorial`)){
    ...
    var embed = new Discord.RichEmbed()
      .setColor(0x3072ff)
      .setDescription(`**[Source Code](https://www.fiverr.com/skyknight47/make-a-custom-discord-bot)**`)
    await message.channel.send(embed)
    var embed = new Discord.RichEmbed()
      .setColor(0x1cff8e)
      .setDescription(`This project was developed by [Nishant S & Mike U].\n__The goal of the project was to provide users with a profesional service of which they can customize and add on to rather than going to a free shi.. service.__`)
